...
async function deleteCurrent(){
  ...
  if(currentSheetKind === 'alarm'){
    ...
    const alarm = DB.getAlarms().find(a=>a.id===id);
    const ok = await showConfirmDialog('Delete alarm?', `Delete "${alarm ? alarm.label : 'this alarm'}"?`);
    if(!ok) return;
...
function buildExpandedChartSvg(type){
  ...
  const W = 640, H = 260, padL = 16, padR = 16, padT = 16, padB = 30;
