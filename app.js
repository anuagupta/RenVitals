...
function saveAlarmFromSheet(){
  ...
  const alarm = { id: currentEditId || genId(), label, time, days, tone, enabled: idx>=0 ? alarms[idx].enabled : true };
...
function deleteCurrent(){
  ...
  if(currentSheetKind === 'alarm'){
    DB.saveAlarms(DB.getAlarms().filter(a=>a.id!==currentEditId));
    renderAlarmsList();
    renderHomeAlarmsPreview();
    scheduleAllAlarms();
  } else if(currentSheetKind === 'new-metric'){
    DB.saveCustomMetrics(DB.getCustomMetrics().filter(m=>m.id!==currentEditId));
    renderAll();
    renderSettingsPanel();
  } else {
...
function renderSettingsPanel(){
  ...
  if(window.VitalsDrive && window.VitalsDrive.isConnected()){
    ...
    $('#drive-status-sub').textContent = 'New entries back up automatically';
    ...
    $('#drive-status-label').textContent = 'Not connected';
...
function wireEvents(){
  ...
  $('#alarms-list').addEventListener('click', (e)=>{
    ...
    if(toggle){
      ...
      if(a){ a.enabled = !a.enabled; DB.saveAlarms(alarms); renderAlarmsList(); renderHomeAlarmsPreview(); scheduleAllAlarms(); }
      return;
    }
