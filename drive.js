...
async function ensureMedicinesHeader(){

  /*
   * Create Medicines tab if it doesn't exist.
   */
  await ensureTab(
    DRIVE_CONFIG.MEDICINES_TAB
  );
...
async function ensureDoseLogHeader(){

  /*
   * Create DoseLog tab if it doesn't exist.
   */
  await ensureTab(
...
async function ensureSpreadsheetReady(){
  ...
  await ensureMedicinesHeader();

  await ensureDoseLogHeader();
...
/* =========================================================================
   ...
   A medicine's schedule (name, dose, times[], days, tone, enabled) syncs
   the same way alarms used to: a plain last-write-wins merge by stable
   medicine ID, no name-based canonicalization. Deletions are tombstoned
   via the Deleted column so a delete made on one device is not silently
   resurrected by the other device's copy.
   ========================================================================= */

function medicineToRow(medicine, deleted){
  return [
    medicine.id || '',
    medicine.name || '',
    medicine.dose || '',
    medicine.time || '',
    JSON.stringify(medicine.days != null ? medicine.days : 'daily'),
    medicine.tone || 'chime',
    medicine.enabled === false ? 'FALSE' : 'TRUE',
    medicine.createdAt || medicine.updatedAt || Date.now(),
...
function rowToMedicine(row){
  ...
  return {
    ...
    createdAt: Number(row[7]) || updatedAt,
    updatedAt,
    deleted:false
  };
}

async function getAllMedicineRows(){
  const range = encodeURIComponent(DRIVE_CONFIG.MEDICINES_TAB + '!A2:J');
...
async function findMedicineRowNumberById(id){
  const range = encodeURIComponent(DRIVE_CONFIG.MEDICINES_TAB + '!A:A');
  const response = await apiFetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}`
  );
...
async function upsertMedicineRow(medicine, deleted){
  ...
  if(!rowNumber){
    const range = encodeURIComponent(DRIVE_CONFIG.MEDICINES_TAB + '!A1:J1');
    await apiFetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,
      {
        method: 'POST',
        headers:{ 'Content-Type': 'application/json' },
        body: JSON.stringify({ values:[ medicineToRow(medicine, deleted) ] })
      }
    );
    return;
  }

  const range = encodeURIComponent(DRIVE_CONFIG.MEDICINES_TAB + `!A${rowNumber}:J${rowNumber}`);
  await apiFetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}?valueInputOption=RAW`,
    {
      method: 'PUT',
...
async function syncMedicines(){
  ...
  remoteRows.forEach(row => {
    ...
    if(medicine && medicine.id){
      const existing = remote.get(medicine.id);
      if(!existing || Number(medicine.updatedAt || 0) >= Number(existing.updatedAt || 0)){
        remote.set(medicine.id, medicine);
      }
    }
  });
...
/* =========================================================================
   ...
   Records whether a specific scheduled dose (one medicine, one date, one
   time) has been marked taken or skipped. Unlike Medicines/Alarms, the
   local store is an object keyed by a deterministic dose ID
   (medicineId|date|time) rather than an array — that ID is what's used as
   the row's own ID, so two devices marking the same dose taken merge onto
   the same row instead of duplicating. A dose with no row here is simply
   "pending" — reverting a mark back to pending is modeled the same way as
   every other delete in this file: a tombstone via the Deleted column,
   which drops it back out of the local object entirely on merge.
   ========================================================================= */

function doseLogToRow(entry, deleted){
  return [
    entry.id || '',
    entry.medicineId || '',
    entry.date || '',
...
function rowToDoseLog(row){
  ...
  const id = String(row[0]);
  const updatedAt = Number(row[5]) || 0;
...
async function findDoseLogRowNumberById(id){
  const range = encodeURIComponent(DRIVE_CONFIG.DOSELOG_TAB + '!A:A');
  const response = await apiFetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}`
  );
  const rows = response.values || [];
  ...
  if(!rowNumber){
    ...
    await apiFetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,
      {
        method: 'POST',
        headers:{ 'Content-Type': 'application/json' },
        body: JSON.stringify({ values:[ doseLogToRow(entry, deleted) ] })
      }
    );
    return;
  }
...
async function syncNow(force){
  ...
  try{
    ...
    /*
     * THIRD:
     * synchronize medicines, then the dose log that tracks which of
     * today's (and recent) scheduled doses have been marked taken/skipped.
     */
    await syncMedicines();
...
/*
 * Every queued item now carries a `kind` ('entry' | 'metric' | 'medicine' |
 * 'doselog') so one offline queue can carry all four record types. Items
 * queued by an older version of this file have no `kind` field — they are
 * always entries, so `kind || 'entry'` keeps them working after this update.
 */
  ...
  return getQueue()
    .filter(
      item => {
        ...
        let itemId;
        if(itemKind === 'entry'){
          itemId = item.op === 'upsert' ? (item.entry && item.entry.id) : item.id;
        }else if(itemKind === 'metric'){
          itemId = item.op === 'upsert' ? (item.metric && item.metric.id) : item.id;
        }else if(itemKind === 'medicine'){
          itemId = item.op === 'upsert' ? (item.medicine && item.medicine.id) : item.id;
        }else if(itemKind === 'doselog'){
          itemId = item.op === 'upsert' ? (item.entry && item.entry.id) : item.id;
        }else{
          itemId = item.id;
        }
  ...
  try{
    ...
    while(
      ...
      try{
        ...
        }else if(kind === 'doselog'){

          if(
            item.op === 'upsert'
          ){

            await upsertDoseLogRow(
              item.entry,
              false
            );
...
window.VitalsDrive = {
  ...
  init(){
    ...
    /*
     * A local entry/metric/medicine/dose-log record changed — push the offline queue if
