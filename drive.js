...
async function syncNow(force){
   ...
   * Google authorization silently. restoreAuthorizedSession() itself
   * decides (via its own cooldown) whether this is actually attempted —
   * syncNow() never forces a visible sign-in.
   */
  if(!accessToken){
...
window.VitalsDrive = {
  ...
  init(){
    ...
    /*
     * Background sync approximately once per minute. syncNow() itself
     * decides whether an auth restore is due (respecting its cooldown)
     * and whether enough time has passed since the last completed sync,
     * so this interval can stay simple: just ask, every time.
     */
    setInterval(
      async () => {
    ...
    /*
     * Startup: if this device was previously authorized, try ONE silent
     * session restore, then sync. If it wasn't authorized, or the silent
     * restore fails, we simply stay disconnected — no consent screen is
     * ever shown just because the app was opened or refreshed. The user
     * remains free to use the app fully offline and tap "Connect"
